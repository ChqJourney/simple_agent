...
class SkillRuntimeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_agent_resolves_local_skill_and_injects_instructions(self) -> None:
        ...
        self.assertIn("`python_execute` already uses the configured Python runtime automatically.", first_request_messages[0]["content"])
        self.assertIn("prefer the plain `python` command first", first_request_messages[0]["content"])
